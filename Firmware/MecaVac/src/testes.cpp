...
int wifi_flag = 1;
unsigned long mom_connectionIni = 0; //****
unsigned long mom_connectionFin = 0; //****
unsigned long tempo_anteriorW = 0; //****
const unsigned long tempo_wifi = 1000; //****
int conectado = 0;
...
#define DS1307_ADDRESS 0x68 //endereço I2C RTC
byte zero = 0x00;
unsigned long temp_set = 0;

// Variáveis controle de temperatura |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||

#include <OneWire.h>
#include <DallasTemperature.h>
unsigned long tempo_anteriorT = 0;
const unsigned long tempo_temp = 30000;
float ideal = 6.0; // temperatura ideal do refriferador


#define ONE_WIRE_BUS_PIN 26  // declarar o pino de entrada dos sensores

// Setup do OneWire para se comunicar com qualquer dispositivo OneWire
OneWire oneWire(ONE_WIRE_BUS_PIN);

// Passa as referências para a biblioteca DallasTemperature
...
LiquidCrystal_I2C lcd(0x3F,20,4);   //endereço "adaptador" i2C do display
...
void setup() {
  ...
  // inicializa a medição dos sensores de temperatura
  sensors.begin();
  // seta a resolução para 12 bits (Pode ser de 9 a 12 bits -> quanto menor, menos preciso mas mais rápido)
  // essa é a biblioteca que demora mais para rodar
  sensors.setResolution(Probe01, 12);
  sensors.setResolution(Probe02, 12);
  sensors.setResolution(Probe03, 12);
  ledcSetup(canal, freq, resolucao);   // Setup PWM
  ledcAttachPin(peltier, canal);       // Setup PWM
...
// Wifi |||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  if (tempo_atual - tempo_anteriorW >= tempo_wifi){ //****
    if(mom_connectionIni != 0 && mom_connectionFin < 290000){ // verifica se o loop do wifi rodou e depois se ele parou pois foi conectado ou por timeout (300000ms) //****
      conectado = 1; //****
    } //****
    else{ //****
      conectado = 0; //****
    } //****
    tempo_anteriorW = tempo_atual; //****
  } //****
/*
  if(wifi_flag == 1){
    wifi_flag = 0;
    AsyncWiFiManager manager(&server, &dns); //Cria o objeto do gerenciador
    manager.setTimeout(300); //timeout após 5 min (testar e chamar na hora certa)
...
//send = true / false //****
  if (send == true){ //****
    log_name = (String)di + "/" + (String)me + "/" + (String)an + " " + (String)hor + ":" + (String)min + ":" + (String)seg; //****
  } //****
/* -> Modelo padrão log //****
  writeFile(SD, log_name, "MENSAGEM\n"); //****
  appendFile(SD, log_name, "MENSAGEM\n"); //****

-> Log Status //****
  if (est_ini_por4 == 1){ //****
    writeFile(SD, log_name, "Porta 1 fechada\n"); //****
  } //****                                                  //Acrescentar porta 2
  else if (est_ini_por4 == 0){ //****
    writeFile(SD, log_name, "Porta 1 aberta\n"); //****
  } //****
  appendFile(SD, log_name, "Temperatura sensor 1: " + (String)temp1 + "\n"); //****
  appendFile(SD, log_name, "Temperatura sensor 2: " + (String)temp2 + "\n"); //****
  appendFile(SD, log_name, "Temperatura sensor 3: " + (String)temp3 + "\n"); //****
*/
...
// Controle botões/portas/alarme ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||
  ...
  if (analogRead(sens_porta2) < 1000 && est_ini_por2 == 0){  //detecção abertura porta 2 //****
    if ((millis() - tempo_ult_debouce2) > delay_debouce2){ //****
      est_ini_por2 = 1; // aberta //****

      //SERVIDOR: Código 51 (Abre -> Servidor)

      tempo_ult_debouce2 = millis(); //****
    } //****
  } //****
  else{ //****
    if (est_ini_por2 == 1 && analogRead(sens_porta2) > 500 //****
      est_ini_por2 = 0; // fechada //****
      prim_press5 = 1; //****
      prim_press7 = 1; //****
    } //****
  } //****

  if (analogRead(sens_porta2) > 1000 && est_ini_por3 == 0){  //detecção fechamento porta 2 //****
    if ((millis() - tempo_ult_debouce3) > delay_debouce3){ //****
      est_ini_por3 = 1; // fechada //****
    ...
    if (est_ini_por3 == 1 && analogRead(sens_porta2) < 1000){ //****
      est_ini_por3 = 0; // aberta //****
      prim_press6 = 1; //****
    } //****
  } //****
  //*******
  ...
  //******* F.P. (CtrlV da porta1 -> altera lógica?)
  if(est_ini_por2 == 1 && prim_press5 == 1){ //****
    prim_press7 = 1; //****
    prim_press5 = 0; //****
    estado_luz = HIGH;  // primeira leitura de porta aberta -> estado_luz = HIGH //****
    tempo_alarme = tempo_atual; //****
  ...
  if (est_ini_por2 == 1){ //****
    if (tempo_atual >= tempo_alarme + delay_alarme && prim_press7 == 1){ //****
      prim_press7 = 0; //****
      alarme_tocando = HIGH; //****
    } //****
  } //****
 
  if(est_ini_por2 == 0 && prim_press6 == 1){ //****
    prim_press6 = 0; //****
    estado_luz = LOW;  // primeira leitura de porta fechada -> estado_luz = LOW //****
    estado_vermelho = LOW; //****
    estado_verde = HIGH; //****
  ...
  if(conectado == false && estado_1 == false && estado_8 == false && 2.0 < media_tempC < 8.0){ //aciona tela 234 e processos por até 5 minutos 
    estado_234 = true; 
    mom_connectionIni = tempo_atual //****
    AsyncWiFiManager manager(&server, &dns); //Cria o objeto do gerenciador //****
    manager.setTimeout(300); //timeout após 5 min (testar e chamar na hora certa) //****
    Serial.println(); // Imprime o endereço de IP que teremos que entrar no navegador //****
    manager.resetSettings(); //Reseta as configurações do gerenciador //****
    ...
    Serial.print("IP Address: "); //****
    Serial.println(WiFi.localIP()); //****
    mom_connectionFin = tempo_atual - mom_connectionIni  // Verifica o momento que o loop de wifi parou apó seu início //****
  }
  else{
    estado_234 = false;
  }
  ...
  if (estado_234 == true){
    ...
    lcd.setCursor(0, 1);
    lcd.print("  (SSID: CHILLER)   "); //****
    lcd.setCursor(0, 2);
    lcd.print("(AP IP: 192.168.4.1)") //****
    lcd.setCursor(0, 3);
    lcd.print("   SELECT A WIFI    ") //****
    }
    //scrollMessage(3, "CONNECT TO ME (SSID: CHILLER) AND SELECT A WIFI CONNECTION (AP IP: 192.168.4.1)", 500, 20); // não funciona pois o wifi é um loop e a mensagem marquee também
  if (estado_69 == true){
    //estado_69 = false;
    temp_iniRefresh = tempo_atual;
    if(millis() == temp_iniRefresh + temp_refresh){
